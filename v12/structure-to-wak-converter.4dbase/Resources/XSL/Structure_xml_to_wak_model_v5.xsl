	...
	<xsl:template match="/base/table">
		<xsl:variable name="pp" select="position()"/>		
			<xsl:for-each select="./field">
				...
				<!--FIELD-->
				<xsl:value-of select="concat('model.', ../@name, '.', @name)"/>
			...
			<xsl:value-of select="concat('model.', ../../../related_field[@kind = 'destination']/field_ref/table_ref/@name, '.', ../../../@name_1toN)"/>
			...
			<xsl:for-each select="key('related_1_for_table_uuid', @uuid)">
				<xsl:value-of select="concat('model.', ../../../related_field[@kind = 'source']/field_ref/table_ref/@name, '.', ../../../@name_Nto1)"/>
